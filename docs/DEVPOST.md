...
**Neo4j Graph:** 10 vertex types (Host, User, Process, Executable, File, Connection, Event, Correction, Finding, ThreatIntel) and 12 edge types (EXECUTED_ON, SPAWNED, ACCESSED, CONNECTED_TO, LOGGED_ON, MODIFIED, HAS_EXECUTABLE, ON_HOST, CORRECTS, ENRICHED_BY, SAME_BINARY, SUPPORTED_BY). SAME_BINARY joins binary identity (execution artifacts) to filesystem instances (MACB); SUPPORTED_BY materializes findings as graph vertices so the investigation itself is traversable down to the raw artifact. Every entity carries `_origin_*` metadata tracing it back to the raw forensic artifact that produced it. MACB timestamps (born, modified, accessed, changed) preserved on File nodes.
