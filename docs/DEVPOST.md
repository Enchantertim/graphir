...
**MCP Server (Python):** 28 typed tools exposed to Claude Code via the Model Context Protocol. Investigation tools (find_evil with 23 hunt patterns and auto-verified top hits, reconstruct_attack for verified attack-chain narratives with Mermaid diagrams, temporal_integrity for clock-tampering / time-compression detection, query_graph, shortest_path, entity_neighborhood, temporal_chain), ingestion tools (run_plaso, ingest_timeline, ingest_multi for multi-host), verification tools (verify_finding, trace_origin, check_provenance_integrity), correction tools (flag_correction, check_corrections), enrichment tools (lookup_hash, enrich_executables), and output generators (Sigma rules, ATT&CK Navigator, evidence chain, audit report, investigative report MD/PDF/DOCX).
...
**Neo4j Graph:** 11 vertex types (Host, User, Process, Executable, File, Connection, Event, Correction, Finding, ThreatIntel, Artifact) and 13 edge types (EXECUTED_ON, SPAWNED, ACCESSED, CONNECTED_TO, LOGGED_ON, MODIFIED, HAS_EXECUTABLE, ON_HOST, CORRECTS, ENRICHED_BY, SAME_BINARY, SUPPORTED_BY, DERIVED_FROM). SAME_BINARY joins binary identity (execution artifacts) to filesystem instances (MACB); SUPPORTED_BY materializes findings as graph vertices; DERIVED_FROM links every entity to its source Artifact with the exact source-line, so "prove this finding" is a graph traversal (Finding → entity → Artifact → raw line), not a property lookup. Every entity carries `_origin_*` metadata tracing it back to the raw forensic artifact that produced it. MACB timestamps (born, modified, accessed, changed) preserved on File nodes.
