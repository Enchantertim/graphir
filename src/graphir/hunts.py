...
HUNT_QUERIES = {
    ...
    "encoded_commands": {
        ...
        "query": """
            MATCH (p:Process)
            WHERE toLower(p.cmdline) CONTAINS '-e ' OR toLower(p.cmdline) CONTAINS '-en '
               OR toLower(p.cmdline) CONTAINS '-enc ' OR toLower(p.cmdline) CONTAINS '-enco '
               OR toLower(p.cmdline) CONTAINS '-encodedcommand'
